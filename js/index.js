...
function clockRun() {
	...
	$('#start').on('tap', function() {
		...
		db.transaction(function(tx) {
			tx.executeSql('INSERT INTO times (start) VALUES ("' + start_time + '")', [], querySuccessDB, errorDB);
		}, errorDB, successDB);
	...
	$('#end').on('tap', function() {
		alert("END TIME: " + last_id);
		var time = new Date();
		var end_time = time.getTime();
		db.transaction(function(tx) {
			tx.executeSql('UPDATE times SET end="' + end_time + '" WHERE id="' + last_id + '"');
...
function initDB(tx) {
	...
	tx.executeSql('CREATE TABLE IF NOT EXISTS times (id INTEGER PRIMARY KEY AUTOINCREMENT, start, end, description)');
}
